...
<?php
	...
	public static function activate() {
		...
		$current_reminder_schedule = wp_get_schedule( 'mrm_masterclass_reminder_cron' );

		if ( $current_reminder_schedule && 'mrm_every_five_minutes' !== $current_reminder_schedule ) {
			wp_clear_scheduled_hook( 'mrm_masterclass_reminder_cron' );
		}

		if ( ! wp_next_scheduled( 'mrm_masterclass_reminder_cron' ) ) {
			wp_schedule_event( time() + 300, 'mrm_every_five_minutes', 'mrm_masterclass_reminder_cron' );
		}
...
public function mrm_mc_render_critical_error_notice() {
	...
	$feedback_table     = $p . 'mrm_masterclass_feedback';
	...
	$presenter_adds = array(
		...
		'profile_social_links_json'   => "ALTER TABLE {$presenters_table} ADD profile_social_links_json LONGTEXT NULL",
	...
	$registration_adds = array(
		...
		'feedback_url'              => "ALTER TABLE {$registrations_table} ADD feedback_url TEXT NULL",
		...
		'cancelled_at'              => "ALTER TABLE {$registrations_table} ADD cancelled_at DATETIME NULL",
		'cancellation_reason'       => "ALTER TABLE {$registrations_table} ADD cancellation_reason TEXT NULL",
		'feedback_submitted_at'     => "ALTER TABLE {$registrations_table} ADD feedback_submitted_at DATETIME NULL",
	);
...
private function mrm_mc_cancel_url_for_token( $token ) {
	return add_query_arg(
		array(
			'mrm_masterclass_cancel' => rawurlencode( (string) $token ),
		),
		home_url( '/' )
	);
}

private function mrm_mc_feedback_url_for_token( $token ) {
	return add_query_arg(
		array(
			'mrm_masterclass_feedback' => rawurlencode( (string) $token ),
		),
		home_url( '/' )
	);
}

private function mrm_mc_gate_url_for_registration( $registration ) {
	if ( ! empty( $registration->gate_url ) ) {
		return esc_url_raw( $registration->gate_url );
	}
...
private function mrm_mc_presenter_share_card_url( $presenter_id ) {
	$presenter_id = absint( $presenter_id );
	...
	return add_query_arg(
		array(
			'mrm_masterclass_presenter_card' => $presenter_id,
		),
		home_url( '/' )
	);
}

private function mrm_mc_handle_presenter_share_card_request() {
	$presenter_id = absint( $_GET['mrm_masterclass_presenter_card'] ?? 0 );
	...
	$logo    = esc_url_raw( $this->mrm_mc_get_email_logo_url() );

	$name = sanitize_text_field( $presenter->name ?? 'Masterclass Presenter' );
...
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  <defs>
    <linearGradient id="bg" x1="0" y1="0" x2="1" y2="1">
      ...
      <stop offset="100%" stop-color="#f2e4cf"/>
    </linearGradient>
    <clipPath id="photoClip">
      <rect x="95" y="95" width="430" height="430" rx="40" ry="40"/>
    </clipPath>
    <clipPath id="logoClip">
      <rect x="675" y="170" width="330" height="220" rx="28" ry="28"/>
    </clipPath>
  </defs>
  <rect width="1200" height="630" fill="url(#bg)"/>
  <rect x="55" y="55" width="1090" height="520" rx="48" fill="#ffffff" stroke="#dccab0" stroke-width="4"/>
  <rect x="95" y="95" width="430" height="430" rx="40" fill="#f7efe3" stroke="#dccab0" stroke-width="3"/>
  <?php if ( $profile ) : ?>
  <image href="<?php echo esc_attr( $profile ); ?>" x="95" y="95" width="430" height="430" preserveAspectRatio="xMidYMid slice" clip-path="url(#photoClip)"/>
  <?php else : ?>
  <text x="310" y="335" text-anchor="middle" font-family="Georgia, serif" font-size="120" font-weight="700" fill="#9a6a2f"><?php echo esc_html( mb_substr( $name, 0, 1 ) ); ?></text>
  <?php endif; ?>

  <rect x="610" y="95" width="495" height="430" rx="40" fill="#20170f"/>
  <?php if ( $logo ) : ?>
  <image href="<?php echo esc_attr( $logo ); ?>" x="675" y="170" width="330" height="220" preserveAspectRatio="xMidYMid meet" clip-path="url(#logoClip)"/>
  <?php endif; ?>
  <text x="858" y="440" text-anchor="middle" font-family="Georgia, serif" font-size="46" font-weight="700" fill="#ffffff">Masterclass Presenter</text>
</svg>
	<?php
	exit;
}
...
private function mrm_mc_email_template( $heading, $content_html ) {
	$logo = $this->mrm_mc_get_email_logo_url();
...
private function mrm_mc_email_button_html( $url, $label ) {
    $url   = esc_url( $url );
    ...
    return '<a href="' . $url . '" style="display:inline-block;background:#111111;color:#ffffff;text-decoration:none;font-weight:700;padding:13px 20px;border-radius:10px;border:1px solid #111111;text-align:center;min-width:150px;">' . esc_html( $label ) . '</a>';
}


private function mrm_mc_email_secondary_button_html( $url, $label ) {
    $url   = esc_url( $url );
    $label = trim( (string) $label );
    ...
    return '<a href="' . $url . '" style="display:inline-block;background:#ffffff;color:#111111;text-decoration:none;font-weight:700;padding:13px 20px;border-radius:10px;border:1px solid #111111;text-align:center;min-width:150px;">' . esc_html( $label ) . '</a>';
}
...
private function mrm_mc_email_button_row_html( $buttons ) {
    ...
    $html = '<table role="presentation" cellspacing="0" cellpadding="0" border="0" align="center" style="margin:24px auto 0 auto;text-align:center;width:auto;"><tr>';

    foreach ( $valid_buttons as $button ) {
        $html .= '<td align="center" valign="middle" style="padding:6px;white-space:nowrap;">';

        if (
            $button['variant'] === 'cancel'
            || $button['variant'] === 'secondary'
            || stripos( $button['label'], 'cancel' ) !== false
            || stripos( $button['label'], 'unable' ) !== false
        ) {
            $html .= $this->mrm_mc_email_secondary_button_html( $button['url'], $button['label'] );
        } else {
            $html .= $this->mrm_mc_email_button_html( $button['url'], $button['label'] );
        }
...
private function mrm_mc_email_details_box_html( $details_html ) {
	...
	return '<div style="margin-top:16px;padding:16px;border:1px solid #ededed;border-radius:12px;background:#fafafa;font-size:14px;line-height:1.7;color:#222;">
		' . $details_html . '
	</div>';
}
...
private function mrm_mc_piece_url_for_event( $event ) {
    ...
    $event_array = is_array( $event ) ? $event : get_object_vars( $event );

    $direct_keys = array(
        'piece_url',
        'sheet_music_url',
        'masterclass_piece_url',
        'discussion_piece_url',
        'repertoire_url',
        'piece_page_url',
    );

    foreach ( $direct_keys as $key ) {
        if ( ! empty( $event_array[ $key ] ) ) {
            $url = esc_url_raw( (string) $event_array[ $key ] );
            if ( $url !== '' ) {
                return $url;
            }
        }
    ...
    $page_id_keys = array(
        'piece_page_id',
        'sheet_music_page_id',
        'discussion_piece_page_id',
    );

    foreach ( $page_id_keys as $key ) {
        if ( ! empty( $event_array[ $key ] ) ) {
            $permalink = get_permalink( absint( $event_array[ $key ] ) );
            if ( $permalink ) {
    ...
    if ( ! empty( $event_array['metadata_json'] ) ) {
        $meta = json_decode( (string) $event_array['metadata_json'], true );

        if ( is_array( $meta ) ) {
            foreach ( $direct_keys as $key ) {
                if ( ! empty( $meta[ $key ] ) ) {
                    $url = esc_url_raw( (string) $meta[ $key ] );
                    if ( $url !== '' ) {
                        return $url;
                    }
                }
            ...
            foreach ( $page_id_keys as $key ) {
                if ( ! empty( $meta[ $key ] ) ) {
                    $permalink = get_permalink( absint( $meta[ $key ] ) );
                    if ( $permalink ) {
                        return esc_url_raw( $permalink );
                    }
                }
    ...
    return (string) apply_filters( 'mrm_masterclass_piece_url_for_event', '', $event );
}
...
public function handle_cross_plugin_email_test( $slug, $to, &$results ) {
    ...
    $results[ $slug ] = $this->mrm_mc_send_email_recorded(
        'email_test_' . $slug,
        $to,
        '[TEST] ' . $subject,
        (string) $preview['html'],
...
private function mrm_mc_confirmation_email_body( $event, $presenter, $registration ) {
	...
	$intro = '<p>You have successfully registered for the masterclass ' . esc_html( $event->title ?? '' ) . ' with ' . esc_html( $presenter->name ?? ( $presenter->presenter_name ?? '' ) ) . '.</p>'
		. $this->mrm_mc_piece_preparation_sentence_html( $event );
...
private function mrm_mc_reminder_email_body( $event, $presenter, $registration, $window_label ) {
	...
	$intro = '<p>This is a reminder for your upcoming masterclass.</p>'
		. $this->mrm_mc_piece_preparation_sentence_html( $event )
		. '<p><strong>Cancellations must be submitted at least 24 hours in advance to receive a refund. Refunds will not be issued for cancellations that occur within 24 hours of the masterclass time.</strong></p>';
...
private function mrm_mc_send_presenter_event_confirmation( $event_id ) {
	...
	$sent = $this->mrm_mc_send_email_recorded(
		...
		'Your upcoming masterclass has been confirmed',
		$body,
		$event_id,
		null
	);
...
private function mrm_mc_feedback_request_email_body( $event, $presenter, $registration ) {
	...
	$intro = '<p>Please rate the masterclass and share any comments you would like us to see.</p>';

	$details = $this->mrm_mc_email_event_details_html( $event, $presenter, $registration );

	$content = $intro
		. $this->mrm_mc_email_details_box_html( $details )
		. $this->mrm_mc_email_button_html( $feedback_url, 'Rate Your Masterclass' );

	return $this->mrm_mc_email_template( 'How was your masterclass?', $content );
}

private function mrm_mc_get_presenter_for_event( $event ) {
	global $wpdb;
	$presenter_id = absint( $event->presenter_id ?? 0 );
	if ( $presenter_id <= 0 ) return (object) array();
	$table = $this->t( 'mrm_masterclass_presenters' );
	if ( ! $this->mrm_mc_table_exists( $table ) ) return (object) array();
	$row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$table} WHERE id = %d LIMIT 1", $presenter_id ) );
	return $row ? $row : (object) array();
}

private function mrm_mc_feedback_received_email_body( $event, $presenter, $registration, $rating = '', $comment = '' ) {
	$details = '<div><strong>Masterclass:</strong> ' . esc_html($event->title ?? '') . '</div>'
		. '<div><strong>Presenter:</strong> ' . esc_html($presenter->name ?? ( $presenter->presenter_name ?? '' ) ) . '</div>'
		. '<div><strong>Rating:</strong> ' . esc_html((string)$rating) . '</div>'
		. '<div><strong>Comment:</strong> ' . esc_html((string)$comment) . '</div>';
	$content = '<p>Masterclass feedback has been submitted.</p>' . $this->mrm_mc_email_details_box_html($details);
	return $this->mrm_mc_email_template('Masterclass Feedback Received', $content);
}

private function mrm_mc_event_update_email_body( $event, $presenter, $registration ) {
	$gate = esc_url( $this->mrm_mc_gate_url_for_registration( $registration ) );

	$intro = '<p>A Masterclass you registered for had its date and time changed. Please review the information below and ensure that you are available for the new time.</p>';
...
private function mrm_mc_refund_completed_email_body( $event, $registration, $amount_cents ) {
	$intro = '<p>Your masterclass refund has been submitted successfully. It may take 3 to 5 business days for the funds to return to your account.</p>';
	...
	return $this->mrm_mc_email_template( 'Masterclass Refund Successful', $content . $this->mrm_mc_email_button_html( home_url( '/contact/' ), 'Contact Support' ) );
}
...
private function mrm_mc_event_cancelled_email_body( $event, $registration, $amount_cents, $refund_status ) {
	...
	return $this->mrm_mc_email_template( 'Masterclass Cancelled', $content . $this->mrm_mc_email_button_html( home_url( '/contact/' ), 'Contact Support' ) );
}
...
private function mrm_mc_send_event_update_notices( $event_id ) {
	...
	foreach ( $registrations as $registration ) {
		if ( $this->mrm_mc_send_email_recorded( 'event_updated', $registration->email, 'Masterclass Updated', $this->mrm_mc_event_update_email_body( $event, $presenter, $registration ), $event->id, $registration->id ) ) { $count++; }
	}
...
private function mrm_mc_sync_google_for_event_id( $event_id ) {
	global $wpdb;

	$event_id = absint( $event_id );

	if ( $event_id <= 0 ) {
		return new WP_Error( 'event_missing_id', 'Masterclass event ID is missing.' );
	}
	...
	if ( ! $this->mrm_mc_table_exists( $events_table ) || ! $this->mrm_mc_table_exists( $presenters_table ) ) {
		return new WP_Error( 'event_table_missing', 'Required Masterclass database tables are missing.' );
	}

	$event = $this->mrm_mc_get_event( $event_id );

	if ( ! $event ) {
		return new WP_Error( 'event_not_found', 'Masterclass event could not be found.' );
	}
	...
	if ( ! $presenter ) {
		$wpdb->update(
			...
			$this->mrm_mc_filter_data_for_table(
				$events_table,
				array(
					'google_last_error' => 'The selected presenter could not be found.',
					'updated_at'        => $this->now(),
				)
			),
			array( 'id' => $event_id )
	...
	if ( is_wp_error( $google_result ) ) {
		$wpdb->update(
			...
			$this->mrm_mc_filter_data_for_table(
				$events_table,
				array(
					'google_last_error' => $google_result->get_error_message(),
					'updated_at'        => $this->now(),
				)
			),
			array( 'id' => $event_id )
		...
		$this->mrm_mc_debug_log(
			'Masterclass Google sync failed.',
			array(
				'event_id' => $event_id,
				'error'    => $google_result->get_error_message(),
			)
	...
	$update_data = array(
		'google_event_id'   => sanitize_text_field( $google_result['id'] ?? ( $event->google_event_id ?? '' ) ),
		'google_meet_url'   => $meet_url,
		'online_link'       => $meet_url,
		'google_last_error' => '',
		'updated_at'        => $this->now(),
	);

	if ( $this->mrm_mc_column_exists( $events_table, 'calendar_id' ) ) {
	...
	return array(
		'event_id'  => $event_id,
		'google_id' => sanitize_text_field( $google_result['id'] ?? '' ),
		'meet_url'  => $meet_url,
	);
}
...
private function mrm_mc_google_update_event( $event, $presenter_email ) {
	...
	$body = $this->mrm_mc_google_event_body( $event, $presenter_email, $needs_meet_request );

	$url = 'https://www.googleapis.com/calendar/v3/calendars/' . rawurlencode( $calendar_id ) . '/events/' . rawurlencode( $event->google_event_id );
	$url = add_query_arg(
		array(
			'conferenceDataVersion' => 1,
			'sendUpdates'           => 'all',
...
private function mrm_mc_google_cancel_event( $google_event_id, $calendar_id = '' ) {
	...
	$url = 'https://www.googleapis.com/calendar/v3/calendars/' . rawurlencode( $calendar_id ) . '/events/' . rawurlencode( $google_event_id );
...
private function mrm_mc_cancel_unconfirmed_payment_intent( $payment_intent_id ) {
	...
	$intent = $this->mrm_mc_stripe_request( 'GET', 'payment_intents/' . rawurlencode( $payment_intent_id ) );

	if ( is_wp_error( $intent ) ) {
		return $intent;
	}

	$status = sanitize_key( $intent['status'] ?? '' );

	if ( 'succeeded' === $status ) {
		return new WP_Error( 'mrm_masterclass_payment_already_received', 'This payment has already succeeded.', array( 'payment_received' => true ) );
	}

	if ( in_array( $status, array( 'canceled', 'requires_payment_method', 'requires_confirmation', 'requires_action', 'requires_capture' ), true ) ) {
		if ( 'canceled' === $status ) {
			return true;
		}

		return $this->mrm_mc_stripe_request( 'POST', 'payment_intents/' . rawurlencode( $payment_intent_id ) . '/cancel' );
	}
...
private function mrm_mc_release_existing_customer_holds(
	...
	$holds = $this->mrm_mc_with_event_seat_lock(
		$event_id,
		function () use ( $wpdb, $table, $event_id, $email_hash ) {
			$rows = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT * FROM {$table}
					 WHERE event_id = %d
			...
			foreach ( (array) $rows as $row ) {
				...
				if ( false === $updated ) {
					return new WP_Error( 'mrm_masterclass_hold_release_failed', 'One or more previous seat holds could not be prepared for release.' );
				}
			}
	...
	foreach ( (array) $holds as $hold ) {
		...
		if ( '' === $payment_intent_id ) {
			$released = $this->mrm_mc_update_seat_hold_row( $event_id, $hold_id, array( 'status' => 'released', 'error_message' => '' ) );
			if ( ! $released ) {
				$errors[] = 'Seat hold ' . $hold_id . ' could not be released.';
			}
			continue;
		}
		...
		if ( is_wp_error( $cancel_result ) ) {
			...
			$this_payment_received = is_array( $error_data ) && ! empty( $error_data['payment_received'] );
			if ( $this_payment_received ) {
				$payment_received = true;
			}
			$restored = $this->mrm_mc_update_seat_hold_row( $event_id, $hold_id, array( 'status' => $this_payment_received ? 'payment_received' : 'reserved', 'error_message' => $cancel_result->get_error_message() ) );
			if ( ! $restored ) {
		...
		$released = $this->mrm_mc_update_seat_hold_row( $event_id, $hold_id, array( 'status' => 'released', 'error_message' => '' ) );
		if ( ! $released ) {
			$wpdb->update( $table, array( 'status' => 'released', 'error_message' => '', 'updated_at' => $this->now() ), array( 'id' => $hold_id ) );
		}
	}

	if ( $payment_received ) {
		return new WP_Error( 'mrm_masterclass_existing_payment_pending', 'A payment for this Masterclass has already been received and is being finalized.', array( 'payment_received' => true ) );
	}

	if ( ! empty( $errors ) ) {
		return new WP_Error( 'mrm_masterclass_previous_hold_release_failed', implode( ' | ', array_unique( $errors ) ) );
	}
	...
	if ( $rows ) {
		foreach ( $rows as $row ) {
			...
			if ( $sent ) {
				...
				$wpdb->update(
					$regs_table,
					$this->mrm_mc_filter_data_for_table( $regs_table, $update ),
					array( 'id' => absint( $row->id ) )
				);
...
public function handle_save_event() {
	...
	$this->mrm_mc_debug_log(
		'Masterclass event time saved from admin event form.',
		array(
			'event_id'   => absint( $event_id ),
	...
	if ( is_wp_error( $google_result ) ) {
		if ( $is_edit ) {
			$this->mrm_mc_send_event_update_notices( $event_id );
		}
...
private function mrm_mc_generate_session_page_for_event_id( $event_id ) {
	...
	if ( ! empty( $presenter_page_url ) ) {
		$content .= '<p><a href="' . esc_url( $presenter_page_url ) . '" style="display:inline-block;background:#20170f;color:#fff;padding:12px 18px;border-radius:999px;text-decoration:none;font-weight:900;">More About the Presenter</a></p>';
	}

	if ( ! empty( $contact_bits ) ) {
		$content .= '<div style="background:#f7efe3;border-radius:18px;padding:18px;margin:22px 0 0;"><strong>Presenter Contact</strong><br>' . implode( '<br>', $contact_bits ) . '</div>';
	}
...
public function mrm_mc_print_presenter_page_share_meta() {
	...
	$presenter = $wpdb->get_row(
		$wpdb->prepare(
			"SELECT * FROM {$presenters_table} WHERE presenter_page_id = %d LIMIT 1",
			absint( $post->ID )
		)
	);
	...
	$title       = 'Masterclass Presenter — ' . sanitize_text_field( $presenter->name ?? '' );
	$description = wp_strip_all_tags( $presenter->long_description ?? ( $presenter->bio ?? '' ) );
	$description = wp_trim_words( $description, 28, '...' );
	$page_url    = get_permalink( $post->ID );
	$image_url   = $this->mrm_mc_presenter_share_card_url( $presenter->id );

	echo "\n<!-- MRM Masterclass Presenter Share Meta -->\n";
...
private function mrm_mc_generate_presenter_page_for_id( $presenter_id ) {
	...
	if ( ! empty( $presenter->long_description ) ) {
		$content .= '<section style="background:#fff;border:1px solid #dccab0;border-radius:28px;padding:clamp(22px,4vw,34px);margin:28px 0;line-height:1.75;font-size:16px;box-shadow:0 14px 34px rgba(32,23,15,.08);">';
	...
	if ( ! empty( $upcoming_sessions ) ) {
		foreach ( $upcoming_sessions as $session ) {
			...
			$seats_label      = absint( $available_seats ) . ' spots available';
			...
			$content .= '<div style="display:flex;flex-direction:column;gap:10px;min-width:190px;">';
			...
			if ( ! empty( $session_page_url ) ) {
				$content .= '<a href="' . esc_url( $session_page_url ) . '" style="display:inline-flex;align-items:center;justify-content:center;background:#20170f;color:#fff;padding:13px 18px;border-radius:999px;text-decoration:none;font-weight:900;">Learn More About This Masterclass</a>';
			}
		...
		$content .= '<div style="background:#fffdf9;border:1px solid #eadcc8;border-radius:24px;padding:24px;text-align:center;color:#6d5c45;">No upcoming Masterclass sessions are currently open for this presenter.</div>';
	}
	...
	$page_title = 'Masterclass Presenter';
...
public function handle_resend_reminder() {
	...
	$this->mrm_mc_debug_log(
		'Reminder resend request could not be completed because this action is unavailable.',
		array(
...
public function register_masterclass_gate_query_vars( $vars ) {
	...
	$vars[] = 'mrm_masterclass_cancel';
	$vars[] = 'mrm_masterclass_feedback';
	$vars[] = 'mrm_masterclass_presenter_card';
...
private function mrm_mc_resolve_gate_event_access_source( $event ) {
	...
	if ( $google_start_ts > 0 && $google_end_ts > $google_start_ts ) {
		...
		$this->mrm_mc_debug_log(
			'Masterclass access gate using Google Calendar event as read-only source of truth.',
			array(
				'event_id' => absint( $event->id ?? 0 ),
				'source'   => 'google_read_only',
			)
		);
	}
...
private function mrm_mc_get_event_for_registration( $registration ) {
	global $wpdb;

	$events_table = $this->t( 'mrm_masterclass_events' );

	if ( ! $registration || empty( $registration->event_id ) || ! $this->mrm_mc_table_exists( $events_table ) ) {
		return null;
	}
...
private function mrm_mc_handle_cancel_request() {
	$token = sanitize_text_field( wp_unslash( $_GET['mrm_masterclass_cancel'] ?? '' ) );

	if ( '' === $token || strlen( $token ) < 20 ) {
		$this->mrm_mc_render_gate_page(
			'Invalid Cancellation Link',
			'This cancellation link is invalid. Please use the link from your Masterclass email.'
		);
		exit;
	}
	...
	if ( ! $registration ) {
		$this->mrm_mc_render_gate_page(
			'Invalid Cancellation Link',
			'This cancellation link was not found. Please use the link from your Masterclass email.'
		);
		exit;
	}

	$event = $this->mrm_mc_get_event_for_registration( $registration );

	if ( ! $event ) {
		$this->mrm_mc_render_gate_page(
			'Masterclass Not Found',
			'This registration is valid, but the linked Masterclass could not be found. Please contact support.'
		);
		exit;
	}

	$access_source = method_exists( $this, 'mrm_mc_resolve_gate_event_access_source' )
		? $this->mrm_mc_resolve_gate_event_access_source( $event )
		: array(
			'start_ts' => strtotime( $event->start_time . ' UTC' ),
			'end_ts'   => strtotime( $event->end_time . ' UTC' ),
			'title'    => sanitize_text_field( $event->title ?? 'Masterclass' ),
	...
	if ( 'refunded' === sanitize_key( $registration->payment_status ) ) {
		$this->mrm_mc_render_gate_page(
			'Enrollment Already Cancelled',
			'This enrollment has already been cancelled and refunded.'
		);
		exit;
	}

	if ( 'POST' === strtoupper( $_SERVER['REQUEST_METHOD'] ?? '' ) ) {
		check_admin_referer( 'mrm_masterclass_cancel_enrollment_' . absint( $registration->id ) );
		...
		foreach ( array_keys( $options ) as $key ) {
			if ( ! empty( $_POST[ 'reason_' . $key ] ) ) {
				$selected[] = $options[ $key ];
			}
		}

		$other       = sanitize_textarea_field( wp_unslash( $_POST['other_reason'] ?? '' ) );
		$reason_text = implode( '; ', $selected );

		if ( '' !== $other ) {
			$reason_text .= ( '' !== $reason_text ? '; ' : '' ) . 'Other: ' . $other;
		}
		...
		$refund = $this->mrm_mc_refund_registration( $registration, $event, 'customer_cancelled_before_start' );

		if ( is_wp_error( $refund ) ) {
			$this->mrm_mc_render_gate_page(
				'Refund Could Not Be Completed',
				'Your cancellation request was received, but the automatic refund could not be completed. Please contact support through the contact form on the website.'
			);
			exit;
		}
	...
	$form .= '<legend><strong>Why can you no longer attend?</strong></legend>';
	...
	$form .= '</fieldset>';
	$form .= '<button class="mrm-masterclass-gate-button" type="submit">Cancel Registration</button>';
	$form .= '</form>';
...
private function mrm_mc_handle_feedback_request() {
	$token = sanitize_text_field( wp_unslash( $_GET['mrm_masterclass_feedback'] ?? '' ) );

	if ( '' === $token || strlen( $token ) < 20 ) {
		$this->mrm_mc_render_gate_page(
			'Invalid Feedback Link',
			'This feedback link is invalid. Please use the link from your Masterclass email.'
		);
		exit;
	}
	...
	if ( ! $registration ) {
		$this->mrm_mc_render_gate_page(
			'Invalid Feedback Link',
			'This feedback link was not found. Please use the link from your Masterclass email.'
		);
		exit;
	}
	...
	if ( $end_ts <= 0 || $now_ts < $end_ts ) {
		$this->mrm_mc_render_gate_page(
			'Feedback Opens After the Masterclass',
			'This feedback form will open after the Masterclass has ended.'
		);
		exit;
	}

	if ( $now_ts > $end_ts + DAY_IN_SECONDS ) {
		$this->mrm_mc_render_gate_page(
			'Feedback Form Has Closed',
			'This feedback form has closed. Thank you for attending the Masterclass.'
		);
		exit;
	}
	...
	$feedback_table = $this->t( 'mrm_masterclass_feedback' );
	$regs_table     = $this->t( 'mrm_masterclass_registrations' );

	if ( ! $this->mrm_mc_table_exists( $feedback_table ) ) {
		$this->mrm_mc_render_gate_page(
			'Feedback Temporarily Unavailable',
			'The feedback form could not load right now. Please try again later.'
		);
		exit;
	}

	if ( ! empty( $registration->feedback_submitted_at ) ) {
		$this->mrm_mc_render_gate_page(
	...
	if ( 'POST' === strtoupper( $_SERVER['REQUEST_METHOD'] ?? '' ) ) {
		check_admin_referer( 'mrm_masterclass_feedback_' . absint( $registration->id ) );

		$options  = $this->mrm_mc_feedback_reason_options();
		$selected = array();

		foreach ( array_keys( $options ) as $key ) {
			if ( ! empty( $_POST[ 'feedback_' . $key ] ) ) {
				$selected[] = $options[ $key ];
			}
		}

		$rating              = max( 1, min( 5, absint( $_POST['rating'] ?? 0 ) ) );
		$other_reason        = sanitize_textarea_field( wp_unslash( $_POST['other_reason'] ?? '' ) );
		$additional_comments = sanitize_textarea_field( wp_unslash( $_POST['additional_comments'] ?? '' ) );
		...
		$wpdb->insert(
			...
			array(
				...
				'rating'               => $rating,
				'selected_reasons'     => wp_json_encode( array_values( $selected ) ),
		...
		$wpdb->update(
			...
			$this->mrm_mc_filter_data_for_table(
				$regs_table,
				array(
					'feedback_submitted_at' => $this->now(),
					'updated_at'            => $this->now(),
				)
			),
			array( 'id' => absint( $registration->id ) )
		...
		$presenter = $this->mrm_mc_get_presenter_for_event( $event );
		$feedback_body = $this->mrm_mc_feedback_received_email_body( $event, $presenter, $registration, (string) $rating, $additional_comments );
		$admin_email = sanitize_email( (string) get_option( 'admin_email' ) );
		$presenter_email = sanitize_email( (string) ( $presenter->email ?? $event->presenter_email ?? '' ) );
		if ( $presenter_email && is_email( $presenter_email ) ) {
			$this->mrm_mc_send_email_recorded( 'feedback_received', $presenter_email, 'Masterclass Feedback Received', $feedback_body, $event->id, $registration->id );
		}
		if ( $admin_email && is_email( $admin_email ) ) {
			$this->mrm_mc_send_email_recorded( 'feedback_received', $admin_email, 'Masterclass Feedback Received', $feedback_body, $event->id, $registration->id );
		}
	...
	$form .= '<fieldset style="border:1px solid #dccab0;border-radius:18px;padding:18px;margin:18px 0;">';
	$form .= '<legend><strong>How would you rate this Masterclass?</strong></legend>';
	$form .= '<div style="display:flex;gap:10px;flex-wrap:wrap;">';

	for ( $i = 1; $i <= 5; $i++ ) {
		$form .= '<label style="display:inline-flex;align-items:center;gap:6px;"><input type="radio" name="rating" value="' . esc_attr( $i ) . '"' . checked( 5, $i, false ) . '> ' . esc_html( str_repeat( '★', $i ) ) . '</label>';
	}
	...
	$form .= '<fieldset style="border:1px solid #dccab0;border-radius:18px;padding:18px;margin:18px 0;">';
	$form .= '<legend><strong>What stood out?</strong></legend>';
	...
	$form .= '<button class="mrm-masterclass-gate-button" type="submit">Submit Feedback</button>';
	$form .= '</form>';
...
public function mrm_mc_handle_gate_request() {
	if ( ! empty( $_GET['mrm_masterclass_presenter_card'] ) ) {
		$this->mrm_mc_handle_presenter_share_card_request();
		exit;
	}

	if ( ! empty( $_GET['mrm_masterclass_cancel'] ) ) {
		$this->mrm_mc_handle_cancel_request();
		exit;
	}

	if ( ! empty( $_GET['mrm_masterclass_feedback'] ) ) {
		$this->mrm_mc_handle_feedback_request();
		exit;
	}
...
public function render_events_page() {
	...
	echo '<button type="submit" class="button">Run Masterclass Registration Successful Cron Now</button>';
	...
	if ( $events ) {
		foreach ( $events as $event ) {
			...
			echo '<td>';
			if ( ! empty( $event->google_event_id ) ) {
				echo '<code>' . esc_html( (string) $event->google_event_id ) . '</code>';
				echo '<p class="description" style="margin:4px 0 0;">Google Meet is attached to the Calendar event; students receive protected access links by email.</p>';
			} else {
				echo '—';
			}
			echo '</td>';
	...
	echo '<a class="button button-small" href="' . esc_url( $this->mrm_mc_public_session_page_url( absint( $event->id ) ) ) . '" target="_blank" rel="noopener">View Session Page</a> ';
			echo '<a class="button button-small button-link-delete" href="' . esc_url( $cancel_url ) . '" onclick="return confirm(\'Cancel/delete this Masterclass? Eligible paid registrations will be refunded automatically through Stripe before the event is removed from active listings.\');">Cancel / Delete</a>';
...
public function render_payouts_page() {
	...
	echo '<div class="wrap mrm-masterclass-admin">';
	echo '<h1>Presenter Payouts</h1>';
	echo '<p>Review each presenter balance and record payment only after funds have been sent.</p>';
	...
	echo '<table class="widefat striped">';
	echo '<thead><tr><th>Presenter</th><th>Email</th><th>Stripe Account Status</th><th>Amount Owed</th><th>Amount Paid</th></tr></thead><tbody>';
	...
	echo '<h2 style="margin-top:28px;">Payout Details</h2>';
	echo '<p>Record manual payments made outside Stripe, or create a Stripe transfer using the connected payout account on file.</p>';
	...
	echo '<table class="widefat striped">';
	echo '<thead><tr><th>Select</th><th>Presenter</th><th>Event</th><th>Amount Owed</th><th>Paid on or after</th><th>Payout Status</th><th>Created</th><th>Payment Method</th></tr></thead><tbody>';
	...
	if ( $rows ) {
		foreach ( $rows as $row ) {
			...
			echo '<td>' . esc_html( $row->created_at ) . '</td>';
			echo '<td>';
			echo '<p class="description">Send this payout to the presenter through Stripe using the connected payout account on file.</p>';
			echo '<a class="button button-small" href="' . esc_url( $transfer_url ) . '" onclick="return confirm(\'Send this payout to the presenter through Stripe using the connected payout account on file?\');">Create Stripe Transfer</a> ';
			echo '<p class="description">Record this payout as paid after you have paid the presenter outside of Stripe.</p>';
			echo '<a class="button button-small" href="' . esc_url( $single_paid_url ) . '" onclick="return confirm(\'Record this payout as paid after you have paid the presenter outside of Stripe?\');">Record Manual Payment</a>';
			echo '</td>';
			echo '</tr>';
		}
	} else {
	...
	if ( $rows ) {
		submit_button( 'Record Selected Manual Payments' );
	}
...
public function render_tax_profiles_page() {
	...
	$rows = $wpdb->get_results(
		$wpdb->prepare(
			"SELECT p.*,
			        ...
			        t.is_1099_eligible, t.exclude_from_1099, t.notes, t.updated_at AS tax_updated_at,
	...
	echo '<p>Use this export to review annual presenter compensation totals for tax preparation. Full TIN values should remain in your approved secure tax system, not WordPress.</p>';
	...
	echo '<p class="description">Use this export to support your records; final tax filing should be reviewed through your official tax workflow. The export includes paid presenter compensation only and excludes pending amounts, refunds, platform revenue, Stripe fees, employees, and profiles marked for exclusion.</p>';
	echo '</form>';
	...
	foreach ( $rows as $row ) {
		...
		echo '<p><strong>Payout Email:</strong> ' . esc_html( $row->email ) . '</p>';
		echo '<p><strong>Stripe Account Status:</strong> ' . esc_html( ! empty( $row->stripe_connected_account_id ) ? 'Connected' : 'Not connected' ) . '</p>';
		...
		echo '<p><strong>Last Updated:</strong> ' . esc_html( $row->tax_updated_at ?: 'Not yet updated' ) . '</p>';
		echo '<p class="description">Secure tax identifier reference: <code>presenters.' . esc_html( $row->id ) . '.tin</code></p>';
		...
		echo '<tr><th>Payout Email</th><td><input type="email" name="email" class="regular-text" value="' . esc_attr( $row->tax_email ?: $row->email ) . '"></td></tr>';
		...
		echo '<tr><th>Tax Classification</th><td><select name="tin_type">';
		...
		echo '<tr><th>Tax Form Status</th><td><label><input type="checkbox" name="w9_received" value="1" ' . checked( $row->w9_received, 1, false ) . '> Yes</label></td></tr>';
		...
		echo '<tr><th>Payout Notes</th><td><textarea name="notes" rows="4" class="large-text">' . esc_textarea( $row->notes ) . '</textarea></td></tr>';
		echo '</tbody></table>';
...
public function render_email_log_page() {
	...
	public function register_rest_routes() {
		...
		register_rest_route(
			...
			array(
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => array( $this, 'rest_release_seat_hold' ),
				'permission_callback' => '__return_true',
			)
		);
		...
		$this->mrm_mc_debug_log(
			...
			array(
				'namespace' => self::REST_NAMESPACE,
				'routes'    => 'events,event,presenter,apply-promo,create-payment-intent,verify-payment-intent,finalize-registration,gate-heartbeat,health',
			)
		);
	}
...
public function rest_create_payment_intent( $request ) {
	...
	$event_start_ts = method_exists( $this, 'mrm_mc_event_timestamp_from_local' )
		? $this->mrm_mc_event_timestamp_from_local( $event->start_time, $event->timezone ?? 'America/Phoenix' )
		: strtotime( $event->start_time . ' UTC' );

	if ( $event_start_ts > 0 && time() >= $event_start_ts ) {
		return new WP_Error(
			'masterclass_enrollment_closed',
			'Enrollment for this Masterclass has closed because the Masterclass has already started.',
			array( 'status' => 409 )
		);
	}
	...
	if ( is_wp_error( $old_hold_release ) ) {
		$error_data = $old_hold_release->get_error_data();

		if ( is_array( $error_data ) && ! empty( $error_data['payment_received'] ) ) {
			return new WP_Error( 'mrm_masterclass_existing_payment_pending', 'A payment for this Masterclass has already been received and is being finalized. Do not submit another payment.', array( 'status' => 409, 'payment_received' => true ) );
		}
	...
	$intent = is_array( $result['payment_intent'] ?? null ) ? $result['payment_intent'] : array();
	...
	return rest_ensure_response(
		array(
			...
			'seat_hold_expires_at'    => sanitize_text_field( $seat_hold['expires_at'] ?? '' ),
...
public function rest_finalize_registration( $request ) {
	...
	if ( empty( $registration_result['inserted'] ) ) {
		$registration_id = absint( $registration_result['registration_id'] ?? 0 );
		$existing_registration = $registration_result['registration'] ?? null;

		if ( ! is_object( $existing_registration ) && $registration_id > 0 ) {
			$existing_registration = $wpdb->get_row(
				$wpdb->prepare(
					"SELECT * FROM {$regs_table} WHERE id = %d LIMIT 1",
					$registration_id
				)
			);
		}
		...
		$confirmation_sent = ! empty( $existing_registration->confirmation_sent );

		if ( ! $existing_registration || ! is_object( $existing_registration ) ) {
			$repair_pending = true;
		} else {
			$repair_result = $this->mrm_mc_repair_registration_side_effects( $existing_registration, $event, $payment_intent );
			if ( is_wp_error( $repair_result ) ) {
				$repair_pending = true;
				$this->mrm_mc_debug_log( 'Existing Masterclass registration requires finalization repair.', array( 'registration_id' => $registration_id, 'payment_intent_id' => $payment_intent_id, 'message' => $repair_result->get_error_message() ) );
			} else {
				$confirmation_sent = ! empty( $repair_result['confirmation_email_sent'] );
			}
		}

		if ( $repair_pending ) {
			$this->mrm_mc_schedule_finalization_repair( $registration_id, $payment_intent_id, 0 );
		}

		return rest_ensure_response( array( 'success' => true, 'already_finalized' => true, 'registration_id' => $registration_id, 'gate_url' => $existing_registration ? esc_url_raw( $existing_registration->gate_url ?? '' ) : '', 'confirmation_email_sent' => $confirmation_sent, 'finalization_repair_pending' => $repair_pending, 'message' => $repair_pending ? 'The registration is confirmed. Remaining account records are being completed automatically.' : 'This registration was already finalized.' ) );
	}
	$registration_id = absint( $registration_result['registration_id'] );
	$registration = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$regs_table} WHERE id = %d LIMIT 1", $registration_id ) );
	$repair_pending = false;
	...
	if ( ! $registration || ! is_object( $registration ) ) {
		...
		$repair_result = $this->mrm_mc_repair_registration_side_effects( $registration, $event, $payment_intent );
		if ( is_wp_error( $repair_result ) ) {
			$repair_pending = true;
			$this->mrm_mc_debug_log( 'New Masterclass registration requires finalization repair.', array( 'registration_id' => $registration_id, 'payment_intent_id' => $payment_intent_id, 'message' => $repair_result->get_error_message() ) );
		} else {
			$confirmation_sent = ! empty( $repair_result['confirmation_email_sent'] );
		}
	}
	...
	$gate_url = $registration ? esc_url_raw( $registration->gate_url ?? $gate['url'] ) : esc_url_raw( $gate['url'] );
	...
	$this->mrm_mc_debug_log(
		...
		array(
			...
			'repair_pending' => $repair_pending ? 'yes' : 'no',
		)
	);
	...
	return rest_ensure_response(
		array(
			...
			'finalization_repair_pending' => $repair_pending,
			'message' => $repair_pending ? 'Registration confirmed. Remaining account records are being completed automatically.' : ( $confirmation_sent ? 'Registration confirmed.' : 'Registration confirmed. Your confirmation email could not be sent automatically, but your access link is shown here.' ),
		)
	);
}
